...
UPDATE snapshot_app_users 
  SET dbt_start_from = {{cutoff_timestamp}}
    , dbt_updated_at = {{cutoff_timeestamp}} 
  WHERE dbt_start_from < {{cutoff_timestamp}};

DELETE FROM snapshot_app_users WHERE dbt_start_from < {{cutoff_timestamp}};
